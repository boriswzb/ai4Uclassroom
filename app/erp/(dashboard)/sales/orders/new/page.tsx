...
export default function NewSalesOrderPage() {
  ...
  const handleSubmit = async (saveDraft: boolean = false) => {
    ...
    try {
      const res = await fetch('/erp/api/sales/orders', {
        method: 'POST',
  ...
  return (
    <div>
      <div className={styles.header}>
        ...
        <h1 className={styles.title}>新建销售订单</h1>
      </div>
      ...
      <div className={styles.container}>
        ...
        <div className={styles.actions}>
          ...
          <button type="button" className={styles.submitBtn} onClick={() => handleSubmit()}>提交订单</button>
        </div>
      </div>
    </div>
  )
